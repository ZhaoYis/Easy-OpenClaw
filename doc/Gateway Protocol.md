...
`talk.config` with `includeSecrets: true` requires `operator.talk.secrets` (or `operator.admin`).Plugin-registered gateway RPC methods may request their own operator scope, but reserved core admin prefixes (`config.*`, `exec.approvals.*`, `wizard.*`, `update.*`) always resolve to `operator.admin`.Method scope is only the first gate. Some slash commands reached through `chat.send` apply stricter command-level checks on top. For example, persistent `/config set` and `/config unset` writes require `operator.admin`.`node.pair.approve` also has an extra approval-time scope check on top of the base method scope:
...
## [](https://docs.openclaw.ai/gateway/protocol#common-rpc-method-families)Common RPC method families

This page is not a generated full dump, but the public WS surface is broader than the handshake/auth examples above. These are the main method families the Gateway exposes today.`hello-ok.features.methods` is a conservative discovery list built from `src/gateway/server-methods-list.ts` plus loaded plugin/channel method exports. Treat it as feature discovery, not as a generated dump of every callable helper implemented in `src/gateway/server-methods/*.ts`.

### [](https://docs.openclaw.ai/gateway/protocol#system-and-identity)System and identity
...
### [](https://docs.openclaw.ai/gateway/protocol#models-and-usage)Models and usage
...
### [](https://docs.openclaw.ai/gateway/protocol#channels-and-login-helpers)Channels and login helpers
...
### [](https://docs.openclaw.ai/gateway/protocol#messaging-and-logs)Messaging and logs
...
### [](https://docs.openclaw.ai/gateway/protocol#talk-and-tts)Talk and TTS
...
### [](https://docs.openclaw.ai/gateway/protocol#secrets-config-update-and-wizard)Secrets, config, update, and wizard
...
### [](https://docs.openclaw.ai/gateway/protocol#existing-major-families)Existing major families

#### [](https://docs.openclaw.ai/gateway/protocol#agent-and-workspace-helpers)Agent and workspace helpers
...
#### [](https://docs.openclaw.ai/gateway/protocol#session-control)Session control
...
#### [](https://docs.openclaw.ai/gateway/protocol#device-pairing-and-device-tokens)Device pairing and device tokens
...
#### [](https://docs.openclaw.ai/gateway/protocol#node-pairing-invoke-and-pending-work)Node pairing, invoke, and pending work
...
#### [](https://docs.openclaw.ai/gateway/protocol#approval-families)Approval families
...
#### [](https://docs.openclaw.ai/gateway/protocol#other-major-families)Other major families
...
### [](https://docs.openclaw.ai/gateway/protocol#common-event-families)Common event families
...
### [](https://docs.openclaw.ai/gateway/protocol#operator-helper-methods)Operator helper methods
...
## [](https://docs.openclaw.ai/gateway/protocol#agent-delivery-fallback)Agent delivery fallback
...
### [](https://docs.openclaw.ai/gateway/protocol#device-auth-migration-diagnostics)Device auth migration diagnostics
